package com.venkoi.cuentame.app.ui
...
import com.venkoi.cuentame.app.navigation.CuentameNavHost
...
import com.venkoi.cuentame.core.domain.usecase.AppStartState
import com.venkoi.cuentame.feature.onboarding.ui.OnboardingRoute
...
fun CuentameApp(
    ...
    when {
        recoveryState is RestoreStartupState.NotStarted || recoveryState is RestoreStartupState.Recovering -> {
            LoadingContent(stringResource(com.venkoi.cuentame.R.string.state_loading_desc))
        }
...
fun RecoveryRequiredContent(onRetry: () -> Unit) {
    Box(modifier = Modifier.fillMaxSize().padding(16.dp), contentAlignment = Alignment.Center) {
        Column(horizontalAlignment = Alignment.CenterHorizontally) {
            Text(
                text = stringResource(com.venkoi.cuentame.R.string.restore_recovery_required_title),
                style = MaterialTheme.typography.headlineMedium,
                color = MaterialTheme.colorScheme.error
            )
            Spacer(modifier = Modifier.height(8.dp))
            Text(
                text = stringResource(com.venkoi.cuentame.R.string.restore_recovery_required_message),
                style = MaterialTheme.typography.bodyMedium
            )
            Spacer(modifier = Modifier.height(16.dp))
            androidx.compose.material3.Button(onClick = onRetry) {
                Text(text = stringResource(com.venkoi.cuentame.R.string.action_retry_desc))
            }
        }
    }
}
                    ...
                    title = {
                        Text(
                            text = if (isSettingsRoot) {
                                stringResource(com.venkoi.cuentame.R.string.nav_settings)
                            } else {
                                currentTopLevelDestination?.let { stringResource(it.titleTextId) }
                                    ?: stringResource(com.venkoi.cuentame.R.string.app_name)
                            }
                        )
                    },
                    navigationIcon = {
                        if (isSettingsRoot) {
                            IconButton(
                                ...
                                Icon(
                                    ...
                                    contentDescription = stringResource(com.venkoi.cuentame.R.string.action_back)
                                )
                            }
                        }
                    },
                        ...
                        if (isTopLevelDestination && !shouldShowSidebar) {
                            IconButton(
                                ...
                                Icon(
                                    ...
                                    contentDescription = stringResource(com.venkoi.cuentame.R.string.nav_settings)
                                )
                            }
                        }
                    },
    ...
    Surface(
        ...
        Column(Modifier.padding(AppSpacing.md)) {
            ...
            NavigationDrawerItem(
                ...
                label = { Text(stringResource(com.venkoi.cuentame.R.string.nav_settings)) },
