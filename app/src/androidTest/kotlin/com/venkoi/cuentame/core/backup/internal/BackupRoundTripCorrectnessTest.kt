...
import com.venkoi.cuentame.core.backup.BackupSnapshotIntegrityValidator
...
class BackupRoundTripCorrectnessTest {
    ...
    fun materialization_provenance_survives_real_path_round_trip() = runBlocking {
        ...
        val checksumProvider = com.venkoi.cuentame.core.backup.Sha256ChecksumProvider()
        ...
        val attachments = updatedSnapshot.purchaseReceipts.filter { it.attachmentId != null }.map { receipt ->
            ...
            com.venkoi.cuentame.core.model.backup.BackupAttachmentMetadata(
                ...
                sizeBytes = 0,
                checksumSha256 = sha,
                referencedBy = listOf(com.venkoi.cuentame.core.model.backup.BackupAttachmentReference("PURCHASE_RECEIPT", receipt.id))
            )
        }
        ...
        val manifest = BackupManifest(
            ...
            appVersionName = "1.0",
            appVersionCode = 1,
            databaseSchemaVersion = com.venkoi.cuentame.core.common.database.DatabaseSchema.VERSION,
    ...
    private suspend fun seedPurchaseWithParseResult(): PurchaseReceiptId {
        ...
        repository.saveOcrResult(
            ...
            pages = listOf(
                com.venkoi.cuentame.core.model.purchase.ocr.PurchaseInvoiceOcrPage(
                    ocrResultId = "ocr-" + receiptId.value,
                    pageIndex = 0,
                    widthPx = 1000,
                    heightPx = 1000,
                    text = "OCR Text",
                    evidence = com.venkoi.cuentame.core.ocr.api.OcrPageEvidence(1000, 1000, "OCR Text", emptyList())
                )
            ),
