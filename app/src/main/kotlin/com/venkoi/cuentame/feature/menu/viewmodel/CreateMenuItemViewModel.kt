...
class CreateMenuItemViewModel @Inject constructor(
    ...
    fun openComponent(component: DraftMenuComponent? = null) {
        draft.value = draft.value.copy(editingComponentId = component?.draftId, editor = ComponentEditorState(isOpen = true,
            existing = component?.let { com.venkoi.cuentame.core.model.menu.MenuRecipeComponent(MenuRecipeComponentId(it.draftId), MenuRecipeId("draft"), it.ingredientId, it.unitOptionId, it.quantity, it.quantity, 0, java.time.Instant.EPOCH, java.time.Instant.EPOCH) },
