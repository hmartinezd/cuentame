...
object ReorderExport {
    ...
    fun csv(items: List<ReorderItem>): String = buildString {
        ...
        items.filter { it.needsReorder }.sortedWith(compareBy({ it.supplierName ?: "" }, { it.ingredientName }, { it.ingredientId.value })).forEach { i ->
            appendLine(listOf(i.supplierName.orEmpty(), listOfNotNull(i.supplierItem, i.supplierSku).joinToString(" / "), i.ingredientName, n(i.currentBase), i.baseUnit, n(i.parBase), n(i.reorderPointBase), n(i.neededBase), i.purchaseUnit.orEmpty(), n(i.purchaseFactorBase), n(i.purchaseUnits), n(i.purchaseCoverageBase), i.status.name).joinToString(",", transform = ::csv))
        }
    }
