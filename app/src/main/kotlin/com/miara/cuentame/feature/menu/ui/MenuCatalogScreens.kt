...
@Composable fun MenuCatalogDetailRoute(onBack:()->Unit,onOpenMenuItem:(MenuRecipeId)->Unit,vm:MenuCatalogDetailViewModel=hiltViewModel()){
 ...
 LaunchedEffect(s.createdMenuItemId){s.createdMenuItemId?.let{createItemFor=null;picker=null;vm.consumeCreatedMenuItem();onOpenMenuItem(it)}}
 ...
 picker?.let{category->ItemPicker(category,s.availableItems,s.currencyCode,s.busy,{picker=null},{picker=null;createItemFor=category}){recipe->vm.addItem(category,recipe)}}
