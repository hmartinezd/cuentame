...
import com.venkoi.cuentame.core.backup.internal.*
import com.venkoi.cuentame.core.backup.platform.*
...
import com.venkoi.cuentame.core.model.backup.RestoreDatabaseRollbackSnapshot
...
import com.venkoi.cuentame.core.database.repository.RoomInventoryProjectionRebuilder
import com.venkoi.cuentame.core.database.repository.InventoryMovementValidator
import com.venkoi.cuentame.core.database.repository.InventoryMovementHistoryValidator
...
class BackupProductionIntegrationTest {
    ...
    fun setup() {
        ...
        val planner = BackupCreationPlanner(
            localeReconciler,
            mockk(relaxed = true) {
                 coEvery { loadPreferences() } returns com.venkoi.cuentame.core.model.backup.BackupPreferencesDto("SYSTEM", true, "en-US")
            },
        ...
        val restaurantRepository = mockk<com.venkoi.cuentame.core.domain.repository.RestaurantRepository>(relaxed = true) {
            coEvery { getRestaurant() } returns com.venkoi.cuentame.core.model.restaurant.Restaurant(
                com.venkoi.cuentame.core.common.ids.RestaurantId("r1"), "Original", "USD", "en-US", Instant.EPOCH, Instant.EPOCH
            )
        }
        ...
        val preferencesApplier = mockk<RestorePreferencesApplier>(relaxed = true) {
            ...
            coEvery { captureRollback() } returns com.venkoi.cuentame.core.model.backup.BackupPreferencesDto("SYSTEM", true, "en-US")
        }
        ...
        coordinator = BackupRestoreCoordinatorImpl(
            ...
            com.venkoi.cuentame.core.backup.internal.NoOpRestoreFailureInjector()
        )
    }
    ...
    fun production_path_no_attachment_restore() = runBlocking {
        ...
        assertThat(statuses.last()).isInstanceOf(com.venkoi.cuentame.core.domain.repository.BackupOperationStatus.Success::class.java)
    ...
    fun recovery_from_rolling_back_phase_succeeds() = runBlocking {
        ...
        val manifest = com.venkoi.cuentame.core.model.backup.BackupManifest(
            backupFormatVersion = com.venkoi.cuentame.core.backup.api.BackupFormatV1Contract.BACKUP_FORMAT_VERSION, createdAtUtc = "2026-01-01T12:00:00Z", applicationId = "com.venkoi.cuentame",
        ...
        val prevPrefs = com.venkoi.cuentame.core.model.backup.BackupPreferencesDto("SYSTEM", true, "en-US")
    ...
    private fun createTableMetadata(dto: BackupSnapshotDto, schemaVersion: Int): Map<String, com.venkoi.cuentame.core.model.backup.TableMetadata> {
        val counts = mapOf(
        ...
        return expectedTables.associateWith { table ->
             com.venkoi.cuentame.core.model.backup.TableMetadata(
    ...
    fun v1_with_database_attachment_reference_is_rejected() = runBlocking {
        ...
        val prefs = com.venkoi.cuentame.core.model.backup.BackupPreferencesDto("SYSTEM", true, "en-US")
    ...
    private fun buildArchive(
        manifest: com.venkoi.cuentame.core.model.backup.BackupManifest,
        ...
        val manifestJson = codecs.writer.encodeToString<com.venkoi.cuentame.core.model.backup.BackupManifest>(manifest).toByteArray()
        val snapshotJson = codecs.writer.encodeToString<BackupSnapshotDto>(snapshot).toByteArray()
        val prefs = com.venkoi.cuentame.core.model.backup.BackupPreferencesDto("SYSTEM", true, "en-US")
        val prefsJson = codecs.writer.encodeToString<com.venkoi.cuentame.core.model.backup.BackupPreferencesDto>(prefs).toByteArray()
