...
import com.venkoi.restaurantops.core.backup.BackupSnapshotIntegrityValidator
...
class BackupRoundTripCorrectnessTest {
    ...
    fun materialization_provenance_survives_real_path_round_trip() = runBlocking {
        ...
        val checksumProvider = com.venkoi.restaurantops.core.backup.Sha256ChecksumProvider()
        ...
        val attachments = updatedSnapshot.purchaseReceipts.filter { it.attachmentId != null }.map { receipt ->
            ...
            com.venkoi.restaurantops.core.model.backup.BackupAttachmentMetadata(
                ...
                sizeBytes = 0,
                checksumSha256 = sha,
                referencedBy = listOf(com.venkoi.restaurantops.core.model.backup.BackupAttachmentReference("PURCHASE_RECEIPT", receipt.id))
            )
        }
        ...
        val manifest = BackupManifest(
            ...
            appVersionName = "1.0",
            appVersionCode = 1,
            databaseSchemaVersion = com.venkoi.restaurantops.core.common.database.DatabaseSchema.VERSION,
    ...
    private suspend fun seedPurchaseWithParseResult(): PurchaseReceiptId {
        ...
        repository.saveOcrResult(
            ...
            pages = listOf(
                com.venkoi.restaurantops.core.model.purchase.ocr.PurchaseInvoiceOcrPage(
                    ocrResultId = "ocr-" + receiptId.value,
                    pageIndex = 0,
                    widthPx = 1000,
                    heightPx = 1000,
                    text = "OCR Text",
                    evidence = com.venkoi.restaurantops.core.ocr.api.OcrPageEvidence(1000, 1000, "OCR Text", emptyList())
                )
            ),
