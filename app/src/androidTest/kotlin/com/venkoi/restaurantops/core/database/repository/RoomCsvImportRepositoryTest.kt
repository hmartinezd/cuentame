...
import com.venkoi.restaurantops.feature.ingredients.csvimport.domain.NormalizedIngredientData
import com.venkoi.restaurantops.feature.ingredients.csvimport.domain.CsvImportRowStatus
import com.venkoi.restaurantops.feature.ingredients.csvimport.domain.CsvImportService
import com.venkoi.restaurantops.feature.ingredients.csvimport.domain.CsvParser
import com.venkoi.restaurantops.feature.ingredients.csvimport.domain.IngredientColumnMapper
...
class RoomCsvImportRepositoryTest {
    ...
    fun fullPipeline_120CsvRows_previewIsReadOnly_thenCommitSucceeds() = runBlocking {
        db.ingredientCategoryDao().upsert(com.venkoi.restaurantops.core.database.entity.IngredientCategoryEntity(
            "existing-category", restId.value, "Produce", "produce", 0, true, 0, 0, null
        ))
        db.supplierDao().insert(com.venkoi.restaurantops.core.database.entity.SupplierEntity(
            "existing-supplier", restId.value, "Sysco", "sysco", null, null, null, true, 0, 0, null
        ))
        db.inventoryAreaDao().upsert(com.venkoi.restaurantops.core.database.entity.InventoryAreaEntity(
...
Roma Tomatoes,Produce,lb,Case,25,FreshPoint,ABC12,1.30"""
    ...
    fun defaultArea_reusesActiveAreas_andUnknownAreaDoesNotCreateOne() = runBlocking<Unit> {
        ...
        assertThat(unknown.rows.single().issues.map { it.code })
            .contains(com.venkoi.restaurantops.feature.ingredients.csvimport.domain.CsvImportIssueCode.UNKNOWN_AREA)
