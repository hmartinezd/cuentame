-- Close the legacy InventoryArea foreign-UUID metadata disclosure without
-- changing the accepted RPC signature or same-tenant conflict semantics.
...
begin
    ...
    if not found then
        ...
        if current_area.restaurant_id <> p_restaurant_id then
            return query select
                'INVALID_OPERATION'::text, p_entity_id,
                null::bigint, null::bigint, null::bigint, null::bigint;
            return;
        ...
        if current_area.server_version <> p_base_server_version then
            return query select
