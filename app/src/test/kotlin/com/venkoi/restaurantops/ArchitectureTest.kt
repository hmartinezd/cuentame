...
import com.venkoi.restaurantops.core.backup.PackageArchitectureRules
import org.junit.Test
import java.io.File

class ArchitectureTest {

    private val repoRoot = System.getProperty("venkoi.repoRoot")

    @Test
    fun enforcePackageBoundaries() {
        assertWithMessage("System property 'venkoi.repoRoot' must be set. Check build.gradle.kts.")
            .that(repoRoot).isNotNull()
        
        val rootDir = File(repoRoot!!)
        val sourceDir = File(rootDir, "app/src/main/kotlin/com/venkoi/restaurantops")
        ...
        sourceDir.walkTopDown().filter { it.extension == "kt" }.forEach { file ->
            filesInspected++
            val relativePath = file.absolutePath.substringAfter("com/venkoi/restaurantops/")
    ...
    fun ruleModelToDatabaseViolationDetected() {
        val source = "import com.venkoi.restaurantops.core.database.entity.UserEntity\nclass M"
    ...
    fun rulePureBackupToDatabaseViolationDetected() {
        val source = "import com.venkoi.restaurantops.core.database.entity.PurchaseReceiptEntity\nclass V"
    ...
    fun ruleCoreToFeatureViolationDetected() {
        val source = "import com.venkoi.restaurantops.feature.home.HomeViewModel\nclass P"
    ...
    fun aliasedForbiddenImportDetected() {
        val source = "import com.venkoi.restaurantops.core.database.entity.UserEntity as U\nclass M"
        val violations = PackageArchitectureRules.violations("core/model/M.kt", source)
