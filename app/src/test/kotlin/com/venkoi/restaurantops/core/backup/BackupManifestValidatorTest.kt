...
class BackupManifestValidatorTest {
    ...
    @Test
    fun `legacy Venkoi application identity remains restorable`() {
        val legacy = createValidManifest().copy(
            applicationId = BackupApplicationIdentity.LEGACY_VENKOI_APPLICATION_ID
        )
    ...
    fun `rejects overlong attachment list`() {
        ...
        val manifest = createValidManifest().copy(backupFormatVersion = com.venkoi.restaurantops.core.backup.api.BackupFormatV1Contract.BACKUP_FORMAT_VERSION, attachments = manyAttachments)
        val result = BackupManifestValidator.validate(manifest) as BackupValidationResult.Invalid
