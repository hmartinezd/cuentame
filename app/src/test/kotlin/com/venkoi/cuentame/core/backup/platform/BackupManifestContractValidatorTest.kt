...
class BackupManifestContractValidatorTest {
    ...
    fun `invalid attachment metadata fails`() {
        ...
        val manifest = createValidBaseManifest().copy(backupFormatVersion = com.venkoi.cuentame.core.backup.api.BackupFormatV1Contract.BACKUP_FORMAT_VERSION, attachments = listOf(att))
    ...
    fun `duplicate attachment ID fails`() {
        ...
        val manifest = createValidBaseManifest().copy(backupFormatVersion = com.venkoi.cuentame.core.backup.api.BackupFormatV1Contract.BACKUP_FORMAT_VERSION, attachments = listOf(att1, att2))
    ...
    fun `ZIP attachment absent from manifest fails`() {
        val manifest = createValidBaseManifest().copy(backupFormatVersion = com.venkoi.cuentame.core.backup.api.BackupFormatV1Contract.BACKUP_FORMAT_VERSION)
    ...
    fun `bi-directional attachment relationship validation`() {
        ...
        val manifestEmpty = createValidBaseManifest().copy(backupFormatVersion = com.venkoi.cuentame.core.backup.api.BackupFormatV1Contract.BACKUP_FORMAT_VERSION)
