...
import com.venkoi.restaurantops.core.backup.internal.BackupArchiveFingerprinter
import com.venkoi.restaurantops.core.backup.internal.BackupArchiveProcessor
import com.venkoi.restaurantops.core.backup.platform.DefaultBackupArchiveReader
...
class DefaultBackupArchiveReaderTest {
    ...
    fun `inspect schema 2 manifest with schema 3 metadata fails`() = runTest {
        ...
        val manifest = buildManifest().copy(
            ...
            tableMetadata = buildManifest().tableMetadata + ("preparation_recipes" to com.venkoi.restaurantops.core.model.backup.TableMetadata(0, false))
        )
        builder.replaceManifest(manifest).recomputeAllChecksums()
    ...
    fun `inspect core entries accepted in any order`() = runTest {
        ...
        val mJson = jsonCodecs.writer.encodeToString(com.venkoi.restaurantops.core.model.backup.BackupManifest.serializer(), manifest).toByteArray()
        builder.addEntry("manifest.json", mJson)
        builder.addEntry("data/database.json", jsonCodecs.writer.encodeToString(com.venkoi.restaurantops.core.backup.model.BackupSnapshotDto.serializer(), createValidEmptySnapshot()).toByteArray())
        builder.addEntry("preferences/settings.json", jsonCodecs.writer.encodeToString(com.venkoi.restaurantops.core.model.backup.BackupPreferencesDto.serializer(), com.venkoi.restaurantops.core.model.backup.BackupPreferencesDto("SYSTEM", true, "en-US")).toByteArray())
    ...
    fun `inspect with checksums json self-reference fails`() = runTest {
        val manifest = buildManifest()
        val mJson = jsonCodecs.writer.encodeToString(com.venkoi.restaurantops.core.model.backup.BackupManifest.serializer(), manifest).toByteArray()
        val dbJson = jsonCodecs.writer.encodeToString(com.venkoi.restaurantops.core.backup.model.BackupSnapshotDto.serializer(), createValidEmptySnapshot()).toByteArray()
        val pJson = jsonCodecs.writer.encodeToString(com.venkoi.restaurantops.core.model.backup.BackupPreferencesDto.serializer(), com.venkoi.restaurantops.core.model.backup.BackupPreferencesDto("SYSTEM", true, "en-US")).toByteArray()
    ...
    fun `inspect valid attachment-bearing archive returns Ready and Eligible`() = runTest {
        ...
        assertThat(ready.eligibility).isEqualTo(com.venkoi.restaurantops.core.model.backup.BackupRestoreEligibility.Eligible)
    }

    private fun createValidEmptySnapshot() = com.venkoi.restaurantops.core.backup.model.BackupSnapshotDto(
        restaurants = listOf(com.venkoi.restaurantops.core.backup.model.RestaurantBackupDto("rest-1", "Test Rest", "USD", "en-US", 0, 0, null)),
    ...
    private fun buildManifest(): com.venkoi.restaurantops.core.model.backup.BackupManifest {
        val tables = BackupFormatV1Contract.expectedTablesForSchema(2)
            .associateWith { com.venkoi.restaurantops.core.model.backup.TableMetadata(if (it == "restaurants") 1 else 0, it in BackupFormatV1Contract.DERIVED_TABLES) }
        return com.venkoi.restaurantops.core.model.backup.BackupManifest(
            backupFormatVersion = com.venkoi.restaurantops.core.backup.api.BackupFormatV1Contract.BACKUP_FORMAT_VERSION,
