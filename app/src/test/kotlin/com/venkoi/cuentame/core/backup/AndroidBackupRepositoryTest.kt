...
import com.venkoi.cuentame.core.backup.internal.BackupCleanupCoordinator
...
class AndroidBackupRepositoryTest {
    ...
    @Before
    fun setup() {
        val gate = com.venkoi.cuentame.core.backup.internal.RestoreOperationGate()
        gate.updateRecoveryState(com.venkoi.cuentame.core.backup.api.RestoreStartupState.Ready)
    ...
    fun `full successful orchestration sequence`() = runTest {
        val rest = mockk<Restaurant>(relaxed = true) {
            io.mockk.every { id } returns com.venkoi.cuentame.core.common.ids.RestaurantId("r1")
        }
        ...
        coEvery { archiveValidator.validate(any()) } returns com.venkoi.cuentame.core.model.backup.BackupValidationResult.Valid(manifest)
