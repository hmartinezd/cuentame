...
import com.venkoi.restaurantops.core.backup.*
...
class DefaultBackupArchiveValidator @Inject constructor(
    ...
    private fun createTableMetadata(dbDto: BackupSnapshotDto, schemaVersion: Int): Map<String, TableMetadata> {
        ...
        if (schemaVersion >= 9) {
            tables["purchase_invoice_draft_applications"] = TableMetadata(dbDto.purchaseInvoiceDraftApplications.size, false)
            tables["purchase_invoice_line_origins"] = TableMetadata(dbDto.purchaseInvoiceLineOrigins.size, false)
        }
        if (schemaVersion >= 11) {
        ...
        if (schemaVersion >= 14) {
            tables["menus"] = TableMetadata(dbDto.menus.size, false)
        ...
        if (schemaVersion >= 16) {
            tables["sales_imports"] = TableMetadata(dbDto.salesImports.size, false)
            tables["imported_sale_transactions"] = TableMetadata(dbDto.importedSaleTransactions.size, false)
            tables["imported_sale_lines"] = TableMetadata(dbDto.importedSaleLines.size, false)
            tables["sales_import_transaction_refs"] = TableMetadata(dbDto.salesImportTransactionRefs.size, false)
        }
