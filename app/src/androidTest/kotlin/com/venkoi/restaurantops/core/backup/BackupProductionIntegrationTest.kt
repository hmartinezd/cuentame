...
import com.venkoi.restaurantops.core.backup.internal.*
import com.venkoi.restaurantops.core.backup.platform.*
...
import com.venkoi.restaurantops.core.database.repository.RoomInventoryProjectionRebuilder
import com.venkoi.restaurantops.core.database.repository.InventoryMovementValidator
import com.venkoi.restaurantops.core.database.repository.InventoryMovementHistoryValidator
...
class BackupProductionIntegrationTest {
    ...
    fun setup() {
        ...
        val planner = BackupCreationPlanner(
            localeReconciler,
            mockk(relaxed = true) {
                 coEvery { loadPreferences() } returns com.venkoi.restaurantops.core.model.backup.BackupPreferencesDto("SYSTEM", true, "en-US")
            },
        ...
        val restaurantRepository = mockk<com.venkoi.restaurantops.core.domain.repository.RestaurantRepository>(relaxed = true) {
            coEvery { getRestaurant() } returns com.venkoi.restaurantops.core.model.restaurant.Restaurant(
                com.venkoi.restaurantops.core.common.ids.RestaurantId("r1"), "Original", "USD", "en-US", Instant.EPOCH, Instant.EPOCH
            )
        }
        ...
        val preferencesApplier = mockk<RestorePreferencesApplier>(relaxed = true) {
            ...
            coEvery { captureRollback() } returns com.venkoi.restaurantops.core.model.backup.BackupPreferencesDto("SYSTEM", true, "en-US")
        }
        ...
        coordinator = BackupRestoreCoordinatorImpl(
            ...
            com.venkoi.restaurantops.core.backup.internal.NoOpRestoreFailureInjector()
        )
    }
    ...
    fun production_path_no_attachment_restore() = runBlocking {
        ...
        assertThat(statuses.last()).isInstanceOf(com.venkoi.restaurantops.core.domain.repository.BackupOperationStatus.Success::class.java)
    ...
    fun recovery_from_rolling_back_phase_succeeds() = runBlocking {
        ...
        val manifest = com.venkoi.restaurantops.core.model.backup.BackupManifest(
            backupFormatVersion = com.venkoi.restaurantops.core.backup.api.BackupFormatV1Contract.BACKUP_FORMAT_VERSION, createdAtUtc = "2026-01-01T12:00:00Z", applicationId = "com.venkoi.restaurantops",
        ...
        val prevPrefs = com.venkoi.restaurantops.core.model.backup.BackupPreferencesDto("SYSTEM", true, "en-US")
    ...
    private fun createTableMetadata(dto: BackupSnapshotDto, schemaVersion: Int): Map<String, com.venkoi.restaurantops.core.model.backup.TableMetadata> {
        val counts = mapOf(
        ...
        return expectedTables.associateWith { table ->
             com.venkoi.restaurantops.core.model.backup.TableMetadata(
    ...
    fun v1_with_database_attachment_reference_is_rejected() = runBlocking {
        ...
        val prefs = com.venkoi.restaurantops.core.model.backup.BackupPreferencesDto("SYSTEM", true, "en-US")
    ...
    private fun buildArchive(
        manifest: com.venkoi.restaurantops.core.model.backup.BackupManifest,
        ...
        val manifestJson = codecs.writer.encodeToString<com.venkoi.restaurantops.core.model.backup.BackupManifest>(manifest).toByteArray()
        val snapshotJson = codecs.writer.encodeToString<BackupSnapshotDto>(snapshot).toByteArray()
        val prefs = com.venkoi.restaurantops.core.model.backup.BackupPreferencesDto("SYSTEM", true, "en-US")
        val prefsJson = codecs.writer.encodeToString<com.venkoi.restaurantops.core.model.backup.BackupPreferencesDto>(prefs).toByteArray()
