        ...
        when {
            ...
            relativePath.startsWith("feature/") -> {
                ...
                val otherFeatureImport = lines.filter { 
                    it.startsWith("import com.venkoi.cuentame.feature.") && !it.contains(".feature.$featureName.")
                }
                otherFeatureImport.forEach { violations.add(ArchitectureViolation(relativePath, it)) }
                check("com.venkoi.cuentame.app.navigation")
            }
            relativePath.startsWith("core/data/") || relativePath.startsWith("core/database/") -> {
                if (lines.any { it.startsWith("import com.venkoi.cuentame.feature.") }) {
                    violations.add(ArchitectureViolation(relativePath, "com.venkoi.cuentame.feature"))
                }
            }
