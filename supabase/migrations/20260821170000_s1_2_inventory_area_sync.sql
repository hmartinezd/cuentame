create sequence public.inventory_area_change_seq;
...
create unique index inventory_areas_active_normalized_name_idx
    on public.inventory_areas (restaurant_id, normalized_name)
    where deleted_at is null;

create index inventory_areas_restaurant_change_seq_idx
    on public.inventory_areas (restaurant_id, change_seq);
...
create table public.sync_applied_operations (
    ...
    result_server_version bigint not null check (result_server_version >= 1),
...
create policy inventory_areas_select_restaurant_member
on public.inventory_areas
for select
to authenticated
using (
    exists (
        ...
        where membership.restaurant_id = inventory_areas.restaurant_id
          and membership.user_id = (select auth.uid())
    )
);

revoke all on sequence public.inventory_area_change_seq from public, anon, authenticated;
revoke all on table public.inventory_areas from public, anon, authenticated;
revoke all on table public.sync_applied_operations from public, anon, authenticated;

grant select on table public.inventory_areas to authenticated;
