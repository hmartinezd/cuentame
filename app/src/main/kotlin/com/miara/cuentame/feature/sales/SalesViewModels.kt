...
@HiltViewModel class SalesImportListViewModel @Inject constructor(private val restaurants:RestaurantRepository,private val repository:SalesImportRepository,private val coordinator:SalesImportCoordinator,private val consumption:SalesConsumptionPostingCoordinator,private val reader:SalesDocumentReader):ViewModel(){
 ...
 fun dismiss(){transient.value=transient.value.copy(prepared=null,duplicate=null,error=null)};fun consumeSuccess(){transient.value=transient.value.copy(success=false,inventoryNeedsAttention=false)};private fun fail(e:SalesUiError){transient.value=transient.value.copy(busy=false,error=e)}
}
...
@HiltViewModel class SalesImportDetailViewModel @Inject constructor(saved:SavedStateHandle,private val repository:SalesImportRepository,private val consumption:SalesConsumptionPostingCoordinator?):ViewModel(){
 ...
 init{load()};fun retry()=load();fun retryInventory(){viewModelScope.launch{_state.value=_state.value.copy(inventoryImpact=InventoryImpactState.UPDATING);try{val coordinator=consumption;if(coordinator!=null){coordinator.reconcileImport(id);_state.value=_state.value.copy(inventoryImpact=inspectImpact())}else _state.value=_state.value.copy(inventoryImpact=InventoryImpactState.UPDATED)}catch(e:CancellationException){throw e}catch(_:Exception){_state.value=_state.value.copy(inventoryImpact=InventoryImpactState.NEEDS_ATTENTION)}}};private suspend fun inspectImpact():InventoryImpactState{val coordinator=consumption?:return InventoryImpactState.UPDATED;val values=coordinator.inspectImport(id).alignments.values;return when{values.any{it==SalesConsumptionAlignment.NEEDS_RECONCILIATION||it==SalesConsumptionAlignment.HISTORY_CONFLICT||it==SalesConsumptionAlignment.INVALID_SOURCE}->InventoryImpactState.NEEDS_ATTENTION;values.all{it==SalesConsumptionAlignment.NO_EFFECT}->InventoryImpactState.NO_EFFECT;else->InventoryImpactState.UPDATED}};private fun load(){viewModelScope.launch{_state.value=SalesImportDetailState();_state.value=try{val d=repository.getImport(id);if(d==null)SalesImportDetailState(false,error=true) else SalesImportDetailState(false,d,d.summary(),inspectImpact())}catch(e:CancellationException){throw e}catch(_:Exception){SalesImportDetailState(false,error=true)}}}
}
