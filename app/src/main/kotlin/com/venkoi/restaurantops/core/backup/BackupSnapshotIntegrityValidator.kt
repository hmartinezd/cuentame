...
import com.venkoi.restaurantops.core.model.purchase.MatchIntegrityPolicy
import com.venkoi.restaurantops.core.model.salesimport.SalesTransactionSourceIdentity
import java.math.BigDecimal

import com.venkoi.restaurantops.core.domain.service.HistoricalInventoryCostCalculationResult
import com.venkoi.restaurantops.core.domain.service.HistoricalInventoryCostFailure
import com.venkoi.restaurantops.core.domain.service.HistoricalInventoryCostCalculator
import com.venkoi.restaurantops.core.domain.service.HistoricalInventoryCostBoundary
import com.venkoi.restaurantops.core.domain.service.HistoricalInventoryMovement
import com.venkoi.restaurantops.core.domain.service.SourceDocumentIdentity
import com.venkoi.restaurantops.core.domain.service.HistoricalInventoryCostResult
...
object BackupSnapshotIntegrityValidator {
    ...
    private fun validateSalesImports(dto: BackupSnapshotDto, restaurantId: String): BackupSnapshotIntegrityException? {
        ...
        fun publicationMatches(publication: com.venkoi.restaurantops.core.backup.model.MenuPublicationBackupDto, ownerRestaurantId: String, menuId: String, revision: Long, currency: String) =
        ...
        for (transaction in dto.importedSaleTransactions) {
            ...
            fun importMatches(value: com.venkoi.restaurantops.core.backup.model.SalesImportBackupDto) =
    ...
    private fun validateProductionOriginalMovements(
        batch: com.venkoi.restaurantops.core.backup.model.ProductionBatchBackupDto,
        components: List<com.venkoi.restaurantops.core.backup.model.ProductionBatchComponentBackupDto>,
        originalMovements: List<com.venkoi.restaurantops.core.backup.model.InventoryMovementBackupDto>
    ): BackupSnapshotIntegrityException? {
    ...
    private fun validateProductionReversals(
        batch: com.venkoi.restaurantops.core.backup.model.ProductionBatchBackupDto,
        originalMovements: List<com.venkoi.restaurantops.core.backup.model.InventoryMovementBackupDto>,
        reversals: List<com.venkoi.restaurantops.core.backup.model.InventoryMovementBackupDto>
    ): BackupSnapshotIntegrityException? {
    ...
    private fun validateStagedMatches(
        ...
        for (match in dto.purchaseInvoiceLineMatches) {
            ...
            // Status invariants
            val status = try {
                com.venkoi.restaurantops.core.model.purchase.InvoiceLineMatchStatus.valueOf(match.status)
            } catch (_: Exception) {
    ...
    private fun validateMenuRecipes(dto: BackupSnapshotDto, ctx: ValidationContext): BackupSnapshotIntegrityException? {
        for (recipe in dto.menuRecipes) {
            ...
            if (runCatching { com.venkoi.restaurantops.core.model.menu.CashDiscountBehavior.valueOf(recipe.cashDiscountBehavior) }.isFailure) return err(INVALID_MENU_STRUCTURE, "Invalid cash discount behavior")
            // Timestamps
