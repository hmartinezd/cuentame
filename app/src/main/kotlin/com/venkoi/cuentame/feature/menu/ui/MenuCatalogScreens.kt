package com.venkoi.cuentame.feature.menu.ui
...
import com.venkoi.cuentame.core.common.ids.MenuRecipeId
import com.venkoi.cuentame.core.designsystem.util.Formatters
import com.venkoi.cuentame.app.ui.theme.AppSpacing
import com.venkoi.cuentame.core.designsystem.component.*
import com.venkoi.cuentame.core.model.menu.*
import com.venkoi.cuentame.feature.menu.viewmodel.*
import com.venkoi.cuentame.core.common.util.ShareHelper

@OptIn(ExperimentalMaterial3Api::class)
@Composable fun MenuCatalogListRoute(onBack:()->Unit,onOpen:(com.venkoi.cuentame.core.common.ids.MenuId)->Unit,vm:MenuCatalogListViewModel=hiltViewModel()){
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable fun MenuCatalogDetailRoute(onBack:()->Unit,onOpenMenuItem:(MenuRecipeId)->Unit,onCreateMenuItem:(com.venkoi.cuentame.core.common.ids.MenuId,com.venkoi.cuentame.core.common.ids.MenuCategoryId)->Unit,vm:MenuCatalogDetailViewModel=hiltViewModel()){
 ...
 val context=LocalContext.current;var pendingExport by remember{mutableStateOf<com.venkoi.cuentame.core.domain.service.MenuPackageExport?>(null)}
 ...
 val shareTitle=stringResource(R.string.catalog_share_title)
 LaunchedEffect(Unit){vm.exportEvents.collect{event->when(event){is MenuExportEvent.CreateDocument->{pendingExport=event.export;exportLauncher.launch(event.export.suggestedFileName)};is MenuExportEvent.SharePackage->{ShareHelper.shareFile(context,event.export.suggestedFileName,event.export.bytes,"application/json",shareTitle).onFailure{vm.exportWriteFinished(false)}}}}}
 ...
 val beginAddItem:(MenuCategory)->Unit={category->if(s.availableItems.isEmpty())s.menu?.let{onCreateMenuItem(it.id,category.id)} else picker=category}
 Scaffold(topBar={TopAppBar(title={Text(s.menu?.name?:stringResource(R.string.catalog_menu))},navigationIcon={TextButton(onClick=onBack){Text(stringResource(R.string.action_back))}},actions={if(s.menu!=null)TextButton(onClick={editMenu=true}){Text(stringResource(R.string.action_edit))}})}){p->when{ s.loading->Centered(p){CircularProgressIndicator()};s.loadError||s.menu==null->Centered(p){Column(horizontalAlignment=Alignment.CenterHorizontally,verticalArrangement=Arrangement.spacedBy(AppSpacing.md)){Text(stringResource(R.string.catalog_load_failed));Button(onClick=vm::retry){Text(stringResource(R.string.action_retry_desc))}}};else->BoxWithConstraints(Modifier.fillMaxSize().padding(p)){val wide=maxWidth>=720.dp;if(wide)Row(Modifier.fillMaxSize().adaptiveContentWidth(1200.dp).padding(AppSpacing.md),horizontalArrangement=Arrangement.spacedBy(AppSpacing.md)){Column(Modifier.width(320.dp).fillMaxHeight().verticalScroll(rememberScrollState()),verticalArrangement=Arrangement.spacedBy(AppSpacing.md)){MenuSummary(s,{editMenu=true},vm::toggleArchived,{confirmPublish=true},vm::export,vm::share,Modifier.fillMaxWidth());PublicationHistory(s,vm::export,vm::share,Modifier.fillMaxWidth())};CategoryList(s,{addCategory=true},{categoryEdit=it},{deleteCategory=it},beginAddItem,{item->onOpenMenuItem(item.recipe.id)},{removeItem=it},{moveItem=it},vm::moveCategory,vm::moveItem,Modifier.weight(1f).fillMaxHeight().verticalScroll(rememberScrollState()))}else LazyColumn(Modifier.fillMaxSize(),contentPadding=PaddingValues(AppSpacing.md),verticalArrangement=Arrangement.spacedBy(AppSpacing.md)){item{MenuSummary(s,{editMenu=true},vm::toggleArchived,{confirmPublish=true},vm::export,vm::share,Modifier.fillMaxWidth())};item{CategoryList(s,{addCategory=true},{categoryEdit=it},{deleteCategory=it},beginAddItem,{item->onOpenMenuItem(item.recipe.id)},{removeItem=it},{moveItem=it},vm::moveCategory,vm::moveItem,Modifier.fillMaxWidth())};if(s.publications.isNotEmpty())item{PublicationHistory(s,vm::export,vm::share,Modifier.fillMaxWidth())}}}}
 }
 ...
 picker?.let{category->ItemPicker(category,s.availableItems,s.currencyCode,s.busy,{picker=null},{picker=null;s.menu?.let{onCreateMenuItem(it.id,category.id)}}){recipe->picker=null;vm.addItem(category,recipe)}}
