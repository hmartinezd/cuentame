...
import com.miara.cuentame.feature.menu.viewmodel.CreateMenuItemViewModel
...
fun CreateMenuItemRoute(
    ...
    Scaffold(
        ...
        else LazyColumn(Modifier.fillMaxSize().padding(padding).adaptiveContentWidth(760.dp).testTag("create_menu_item_screen"), contentPadding = PaddingValues(AppSpacing.md), verticalArrangement = Arrangement.spacedBy(AppSpacing.lg)) {
            item { AppSectionHeader(stringResource(R.string.menu_basic_information)) }
            item { AppCard(Modifier.fillMaxWidth()) { Column(Modifier.padding(AppSpacing.md), verticalArrangement = Arrangement.spacedBy(AppSpacing.sm)) { OutlinedTextField(name, { name = it }, label = { Text(stringResource(R.string.menu_item_name)) }, modifier = Modifier.fillMaxWidth().testTag("catalog_new_item_name"), enabled = !state.saving); OutlinedTextField(price, { price = it }, label = { Text(stringResource(R.string.menu_selling_price)) }, modifier = Modifier.fillMaxWidth().testTag("catalog_new_item_price"), enabled = !state.saving, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Decimal)) } } }
            item { AppSectionHeader(stringResource(R.string.menu_cash_discount)) }
            item { AppCard(Modifier.fillMaxWidth()) { Column(Modifier.padding(AppSpacing.md)) { CashChoice(state.cashDiscountBehavior == CashDiscountBehavior.APPLY_DEFAULT, stringResource(R.string.menu_cash_use_default_percent, state.defaultDiscountPercent.stripTrailingZeros().toPlainString()), !state.saving) { vm.setCashDiscountBehavior(CashDiscountBehavior.APPLY_DEFAULT) }; CashChoice(state.cashDiscountBehavior == CashDiscountBehavior.NONE, stringResource(R.string.menu_cash_none), !state.saving) { vm.setCashDiscountBehavior(CashDiscountBehavior.NONE) } } } }
            item { AppSectionHeader(stringResource(R.string.menu_recipe_components), trailing = { if (state.ingredients.isNotEmpty()) AppPrimaryButton(onClick = { vm.openComponent() }, enabled = !state.saving, modifier = Modifier.testTag("menu_add_component")) { Icon(Icons.Default.Add, null); Spacer(Modifier.width(AppSpacing.xs)); Text(stringResource(R.string.menu_add_component)) } }) }
            if (state.ingredients.isEmpty()) item { AppCard(Modifier.fillMaxWidth()) { Column(Modifier.padding(AppSpacing.md), verticalArrangement = Arrangement.spacedBy(AppSpacing.sm)) { Text(stringResource(R.string.menu_no_ingredients), style = MaterialTheme.typography.titleMedium); Text(stringResource(R.string.menu_no_ingredients_body), color = MaterialTheme.colorScheme.onSurfaceVariant); Row(horizontalArrangement = Arrangement.spacedBy(AppSpacing.sm)) { OutlinedButton(onClick = onInventory) { Text(stringResource(R.string.menu_go_inventory)) }; TextButton(onClick = onImport) { Text(stringResource(R.string.menu_import_ingredients)) } } } } }
            else if (state.components.isEmpty()) item { AppCard(Modifier.fillMaxWidth()) { Column(Modifier.padding(AppSpacing.md), verticalArrangement = Arrangement.spacedBy(AppSpacing.xs)) { Text(stringResource(R.string.menu_recipe_not_configured), style = MaterialTheme.typography.titleMedium); Text(stringResource(R.string.menu_recipe_not_configured_body), color = MaterialTheme.colorScheme.onSurfaceVariant) } } }
            items(state.components, key = { it.ingredientId.value }) { component -> AppCard(Modifier.fillMaxWidth()) { Row(Modifier.fillMaxWidth().padding(AppSpacing.md), verticalAlignment = Alignment.CenterVertically) { Column(Modifier.weight(1f)) { Text(component.ingredientName, style = MaterialTheme.typography.titleMedium); Text("${Formatters.formatQuantity(component.quantity)} ${component.unitLabel}");if(state.ingredients.firstOrNull{it.id==component.ingredientId}?.defaultAreaId==null)Text(stringResource(R.string.menu_inventory_area_required),color=MaterialTheme.colorScheme.error) }; TextButton(onClick = { vm.removeComponent(component) }, enabled = !state.saving) { Text(stringResource(R.string.menu_remove_component)) } } } }
            state.error?.let { error -> item { Text(operationErrorLabel(error), color = MaterialTheme.colorScheme.error) } }
            if (state.placementRecoveryNeeded) item { Text(stringResource(R.string.menu_create_recovery), color = MaterialTheme.colorScheme.error) }
        }
    }
