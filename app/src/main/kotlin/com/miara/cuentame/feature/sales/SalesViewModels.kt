...
@HiltViewModel class SalesImportDetailViewModel @Inject constructor(saved:SavedStateHandle,private val repository:SalesImportRepository):ViewModel(){
 ...
 init{load()};fun retry()=load();private fun load(){viewModelScope.launch{_state.value=SalesImportDetailState();_state.value=try{val d=repository.getImport(id);if(d==null)SalesImportDetailState(false,error=true) else SalesImportDetailState(false,d,d.summary())}catch(e:CancellationException){throw e}catch(_:Exception){SalesImportDetailState(false,error=true)}}}
}
