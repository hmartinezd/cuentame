...
data class SalesListState(val loading:Boolean=true,val imports:List<SalesImport> = emptyList(),val prepared:PreparedSalesImport?=null,val duplicate:SalesImport?=null,val busy:Boolean=false,val error:SalesUiError?=null,val success:Boolean=false,val noRestaurant:Boolean=false)

@HiltViewModel class SalesImportListViewModel @Inject constructor(private val restaurants:RestaurantRepository,private val repository:SalesImportRepository,private val coordinator:SalesImportCoordinator,private val reader:SalesDocumentReader):ViewModel(){
 ...
 fun confirm(){val prepared=transient.value.prepared?:return;if(transient.value.busy)return;transient.value=transient.value.copy(busy=true,error=null);viewModelScope.launch{try{when(val result=coordinator.commit(prepared)){is SalesImportCommitResult.Imported->transient.value=transient.value.copy(busy=false,prepared=null,success=true);is SalesImportCommitResult.Duplicate->transient.value=transient.value.copy(busy=false,prepared=null,duplicate=result.existing);is SalesImportCommitResult.Failure->transient.value=transient.value.copy(busy=false,error=result.failure.code.ui())}}catch(e:CancellationException){throw e}}}
...
data class SalesImportDetailState(val loading:Boolean=true,val detail:SalesImportDetail?=null,val summary:SalesSummary?=null,val error:Boolean=false)
...
@HiltViewModel class SalesImportDetailViewModel @Inject constructor(saved:SavedStateHandle,repository:SalesImportRepository):ViewModel(){private val id=requireNotNull(saved.get<String>("exportId"));val state=flow{val d=repository.getImport(id);emit(if(d==null)SalesImportDetailState(false,error=true) else SalesImportDetailState(false,d,d.summary()))}.catch{emit(SalesImportDetailState(false,error=true))}.stateIn(viewModelScope,SharingStarted.WhileSubscribed(5_000),SalesImportDetailState())}
@HiltViewModel class SalesTransactionDetailViewModel @Inject constructor(saved:SavedStateHandle,repository:SalesImportRepository):ViewModel(){private val terminal=requireNotNull(saved.get<String>("terminalId"));private val id=requireNotNull(saved.get<String>("transactionId"));val state=flow{emit(repository.getTransaction(terminal,id))}.stateIn(viewModelScope,SharingStarted.WhileSubscribed(5_000),null)}
